...
class VideoCacheServer {
  ...
  Future<void> _loadCacheInfo() async {
    File cacheInfoFile = File(_cacheInfoFile);
    if (cacheInfoFile.existsSync()) {
      try {
        String cacheInfoStr = await cacheInfoFile.readAsString();
        Map<String, dynamic> cacheInfoMap = jsonDecode(cacheInfoStr) as Map<String, dynamic>;
        cacheInfoMap.forEach((key, value) {
          _caches[key] = CacheInfo.fromMap(value as Map<dynamic, dynamic>);
        });
        if (quiet != true) {
          log('Loaded cache info from $_cacheInfoFile');
        }
      } catch (e, s) {
  ...
  // 保存缓存信息的方法
  Future<void> _saveCacheInfo(String? realUrl) async {
    if (_caches.isEmpty) return;
    if (!(_caches[realUrl]?.finished ?? false))  {
      return;
    }

    File cacheInfoFile = File(_cacheInfoFile);
    try {
      Map<String, dynamic> cacheInfoMap = {};
      _caches.forEach((key, value) {
        cacheInfoMap[key] = value.toMap();
      });
      String cacheInfoStr = jsonEncode(cacheInfoMap);
      await cacheInfoFile.writeAsString(cacheInfoStr);
      if (quiet != true) {
        log('Saved cache info to $_cacheInfoFile');
      }
    } catch (e, s) {
      log('Failed to save cache info', error: e, stackTrace: s);
    }
  }
